...
-- Columns: 
--			player_id:   	id of players, auto-generated when
--										player_name is registered.
...
FROM players
LEFT JOIN game ON players.player_id = winner 
...
LEFT JOIN game ON players.player_id = winner 
							 OR players.player_id = loser 
							 OR players.player_id = tie
GROUP BY players.player_id, game.winner, game.tie
...
FROM players
		LEFT JOIN score ON score.player_id = players.player_id
...
--			num_wins.num_wins:  			number of matches the player has won. 
--																Summed from score view
...
SELECT 
	...
	FROM num_wins, num_matches
	WHERE num_wins.player_id = num_matches.player_id
	ORDER BY num_wins.num_wins DESC, num_matches.num_matches DESC;